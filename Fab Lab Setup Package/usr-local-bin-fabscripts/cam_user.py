...
input:
   ...
   *.cmp,*.sol,*.plc,*.sts,*.stc,*.gtl: Gerber
      ...
      for seg in range(len(boundarys[0])):
	    ...
	    if (vert == 1):
         ...
         try:
             ...
             print "We caught the error so cam won't crash - ed baafi"
      c.delete("plot_path")
      #
...
def delframes():
   ...
   if ((find(intext,".cmp") != -1) | (find(intext,".CMP")!= -1) \
      ...
      | (find(intext,".plc")!= -1) | (find(intext,".PLC")!= -1) \
      | (find(intext,".stc")!= -1) | (find(intext,".STC")!= -1) \
      | (find(intext,".gtl")!= -1) | (find(intext,".GTL")!= -1) \
      | (find(intext,".sts")!= -1) | (find(intext,".STS")!= -1)):
...
def send(event):
   ...
   if (find(outtext,".rml") != -1):
      ...
      wdevbtn.update()
      write(event)
      print os.system('lpr -P vinyl %s'%outtext)
      print os.system('rm %s'%outtext)
      wdevbtn.config(text="send to")
      #wdevbtn.update()
   elif (find(outtext,".epi") != -1):
      ...
      wdevbtn.update()
      write(event)
      print os.system('lpr -P rawlaser %s'%outtext)
      print os.system('lpr -P rawlaser2 %s'%outtext)
      print os.system('rm %s'%outtext)
      wdevbtn.config(text="send to")
      #wdevbtn.update()
   else:
...
def openfile():
   ...
   #
   filename = askopenfilename(initialdir='/home/fab/Desktop/Users')
   infile.set(filename)
   read(0)
